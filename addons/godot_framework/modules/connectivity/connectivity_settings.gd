...
@export_range(0, 4096, 1) var http_max_finished_requests := 256
...
@export_range(1, 65536, 1) var request_max_pending := 4096
...
func validate() -> String:
	...
	if http_max_finished_requests < 0:
		return "HTTP finished request limit cannot be negative."
	...
	if request_max_pending < 1:
		return "Correlation request capacity must be at least one."
