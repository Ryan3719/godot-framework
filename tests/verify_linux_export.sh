...
export_dir=$(mktemp -d /tmp/godot-framework-linux-build.XXXXXX)
...
NO_COLOR=1 "$godot_bin" --headless --path "$project_dir" --export-release Linux "$export_dir/godot-framework-test.x86_64" > "$project_dir/export.log" 2>&1
test -x "$export_dir/godot-framework-test.x86_64"
(
	cd "$export_dir"
