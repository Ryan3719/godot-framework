...
The builder sorts paths and normalizes timestamps, permissions, and compression so identical source bytes produce an identical ZIP and SHA-256 checksum. The installation test extracts the package into a temporary clean project, lets the editor import it, and verifies the six enabled foundation modules plus all disabled optional modules.
