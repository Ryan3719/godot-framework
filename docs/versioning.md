...
The supported engine range currently starts at Godot 4.4. CI validates editor/headless behavior plus Linux, macOS, and Windows release exports on both the minimum version, 4.4.1, and the current compatibility version, 4.7.1. These exports exercise the clean addon's default runtime boundary; optional-module integration, Web, and mobile platforms require separate evidence. A future engine version is not supported merely because it parses the project.
