...
The supported engine range currently starts at Godot 4.4. CI validates editor/headless behavior plus Linux, macOS, Windows, and single-threaded Web release exports on both the minimum version, 4.4.1, and the current compatibility version, 4.7.1. These exports exercise the clean addon's default runtime boundary; optional-module integration, threaded Web builds, and mobile platforms require separate evidence. A future engine version is not supported merely because it parses the project.
